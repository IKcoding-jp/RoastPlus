  ...
  const handleShuffle = useCallback(async () => {
    ...
    try {
      ...
      const result = calculateAssignment(
        ...
        displayAssignments,
        pairExclusions,
        shuffleSettings.crossTeamShuffle,
        shuffleSettings.priority
      );
      ...
      if (memberTeamUpdates.length > 0) {
        await Promise.all(memberTeamUpdates.map(({ memberId, teamId }) => updateMemberTeam(userId, memberId, teamId)));
        setMembers((prev) =>
          prev.map((m) => {
