...
export const calculateAssignment = (
  ...
  const sortedRows = rows
    ...
    .sort((a, b) => {
      const aCount = eligibleMembers.filter((m) => !m.excludedTaskLabelIds.includes(a.taskLabelId)).length;
      const bCount = eligibleMembers.filter((m) => !m.excludedTaskLabelIds.includes(b.taskLabelId)).length;
  ...
  /** 行の連続チェック（ハード制約） */
  const hasRowConflict = (memberId: string, taskLabelId: string, level: ConstraintLevel): boolean => {
