...
type UseTableEditingParams = {
  ...
  onUpdateMember: (
    assignment: { teamId: string; taskLabelId: string; memberId: null; assignedDate: string },
    memberId: string
  ) => Promise<void>;
};
...
export function useTableEditing({
  ...
  const handleSaveWidth = useCallback(
    async (width: number, headerName?: string) => {
  ...
  // 行設定（高さ・名前）保存
  const handleSaveRowConfig = useCallback(
    async (height: number, name: string) => {
  ...
  const handleAddMember = useCallback(
    async (taskLabelId: string, teamId: string) => {
