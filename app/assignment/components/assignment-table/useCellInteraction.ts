...
export function useCellInteraction(members: Member[]) {
  ...
  const openContextMenu = useCallback(
    (teamId: string, taskLabelId: string, memberId: string | null) => {
  ...
  const handleCellTouchStart = useCallback(
    (teamId: string, taskLabelId: string, memberId: string | null, e: React.TouchEvent | React.MouseEvent) => {
  ...
  const handleCellClick = useCallback(
    (
      ...
      taskLabelId: string,
      onSwapAssignments: (
        asg1: { teamId: string; taskLabelId: string },
        asg2: { teamId: string; taskLabelId: string }
      ) => Promise<void>
    ) => {
