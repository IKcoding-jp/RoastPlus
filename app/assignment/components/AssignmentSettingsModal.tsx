    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        ...
        <div className="px-4 py-3 flex items-center justify-between bg-ground border-b border-edge">
          <h2 className="font-bold text-ink">詳細設定</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {isDeveloperMode && (
            <div className="p-6">
              ...
              {/* 既存の設定リスト */}
              <div>
                <h4 className="text-sm font-medium mb-2 text-ink">設定済みのペア ({pairExclusions.length}件)</h4>

                {pairExclusions.length === 0 ? (
                  <p className="text-sm text-center py-4 text-ink-muted">設定がありません</p>
                ) : (
                  <ul className="space-y-2">
                    {pairExclusions.map((exclusion) => (
                      <li key={exclusion.id} className="flex items-center justify-between p-3 rounded-lg bg-ground">
