...
export const MemberSettingsDialog: React.FC<Props> = ({ member, taskLabels, isOpen, onClose, onUpdateExclusions }) => {
  if (!isOpen) return null;

  const handleToggle = (taskLabelId: string) => {
    const current = member.excludedTaskLabelIds || [];
    const next = current.includes(taskLabelId) ? current.filter((id) => id !== taskLabelId) : [...current, taskLabelId];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="rounded-xl shadow-xl w-full max-w-md overflow-hidden bg-overlay border border-edge">
        <div className="px-4 py-3 flex justify-between items-center bg-ground border-b border-edge">
          <h3 className="font-bold text-ink">{member.name} さんの設定</h3>
          ...
          <div className="space-y-3 max-h-[60vh] overflow-y-auto">
            {taskLabels.map((task) => {
              ...
              return (
                <label
                  key={task.id}
                  className="flex items-center p-3 rounded-lg border cursor-pointer transition-colors border-edge hover:bg-ground"
                >
                  <Checkbox checked={isExcluded} onChange={() => handleToggle(task.id)} />
                  <span className={`ml-3 font-medium ${isExcluded ? 'text-red-500' : 'text-ink'}`}>
