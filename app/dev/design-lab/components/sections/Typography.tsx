...
const fontFamilies = [
  {
    name: 'Geist Sans',
    className: 'font-sans',
    sample: 'The quick brown fox jumps over the lazy dog - 素早い茶色の狐が怠惰な犬を飛び越える',
  },
  ...
  return (
    <div className="space-y-8">
      ...
      <Card variant="default">
        <div className="p-5">
          <h4 className="text-base font-semibold text-ink mb-4">Font Families</h4>
          <div className="space-y-4">
            {fontFamilies.map((font) => (
              <div key={font.name} className="p-4 rounded-lg bg-ground">
                <p className="text-xs font-medium text-ink-muted uppercase tracking-wide mb-2">{font.name}</p>
                <p className={`text-lg text-ink ${font.className}`}>{font.sample}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <Card variant="default">
        <div className="p-5">
          <h4 className="text-base font-semibold text-ink mb-4">Font Sizes</h4>
          <div className="space-y-3">
            {fontSizes.map((size) => (
              <div key={size.label} className="flex items-baseline gap-4">
                <span className="text-xs text-ink-muted w-20 shrink-0 font-mono">{size.label}</span>
                <span className={`text-ink ${size.className} truncate`}>RoastPlus</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <Card variant="default">
        <div className="p-5">
          <h4 className="text-base font-semibold text-ink mb-4">Font Weights</h4>
          <div className="space-y-3">
            {fontWeights.map((weight) => (
              <div key={weight.label} className="flex items-baseline gap-4">
                ...
                <span className={`text-xl text-ink ${weight.className}`}>Coffee Roasting</span>
              </div>
            ))}
          </div>
        </div>
