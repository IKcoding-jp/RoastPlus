...
// H2: コーヒーヘッダーカード
// - カード上部セクション（ステップ名+水量）の背景をダークブラウンに
// - テキストを白系に変更、水量はオレンジのまま
// - Dropアイコン背景をオレンジに（ブラウン背景で映える）
// - アクセントバーをブラウンのグラデーションに
export default function TimerPatternH2() {
      ...
      <div className="h-[667px] rounded-3xl border border-edge overflow-hidden flex flex-col bg-ground">
        ...
        <div className="flex-1 flex flex-col items-center px-5 pb-3 overflow-y-auto">
          ...
          <div className="w-full rounded-2xl overflow-hidden bg-surface border border-edge shadow-card">
            {/* アクセントバー: ブラウンのグラデーション */}
            <div className="h-[3px] w-full bg-gradient-to-r from-dark to-dark-light" />

            {/* 現在ステップ — 上部セクション（ダークブラウン背景） */}
            <div className="px-4 pt-4 pb-3 bg-dark">
              <div className="flex items-center gap-2.5 mb-3">
                {/* Dropアイコン: オレンジ背景（ブラウン背景で映える） */}
                ...
                {/* ステップ名: 白テキスト */}
                <span className="text-[15px] font-bold text-white leading-tight">{currentStep.title}</span>
              </div>
              ...
              {currentStep.targetTotalWater && (
                <div className="flex items-baseline gap-1 mb-2">
                  ...
                  <span className="text-sm text-white/70 ml-1 font-medium">まで注ぐ</span>
                </div>
              )}

              {/* 説明文: 白系 */}
              <p className="text-[13px] text-white/70 leading-relaxed">{currentStep.description}</p>

              {/* note: 白系の薄め */}
              {currentStep.note && (
                <p className="text-[11px] text-white/40 mt-2 leading-relaxed italic">{currentStep.note}</p>
              )}
            </div>

            {/* 次ステップカウントダウン — 統合下部セクション（白背景のまま） */}
            {nextStep && (
