...
export function setDebugMode(enabled: boolean): void {
  ...
  if (typeof window !== 'undefined') {
    window.__QUIZ_DEBUG_MODE__ = enabled;
    // localStorageに永続化
    try {
...
export function isDebugMode(): boolean {
  if (typeof window !== 'undefined') {
    // windowオブジェクトから取得
    const windowValue = window.__QUIZ_DEBUG_MODE__;
...
export function setDebugDateOffset(days: number): void {
  ...
  if (typeof window !== 'undefined') {
    window.__QUIZ_DEBUG_DATE_OFFSET__ = days;
    // localStorageに永続化
    try {
...
export function getDebugDateOffset(): number {
  if (typeof window !== 'undefined') {
    // windowオブジェクトから取得
    const windowValue = window.__QUIZ_DEBUG_DATE_OFFSET__;
    if (windowValue !== undefined) return windowValue;
    ...
    try {
      ...
      if (stored !== null) {
        ...
        if (!isNaN(offset)) {
          window.__QUIZ_DEBUG_DATE_OFFSET__ = offset;
          return offset;
        }
      }
    } catch {
...
export function resetDebugState(): void {
  ...
  if (typeof window !== 'undefined') {
    window.__QUIZ_DEBUG_MODE__ = false;
    window.__QUIZ_DEBUG_DATE_OFFSET__ = 0;
    // localStorageからも削除
    try {
