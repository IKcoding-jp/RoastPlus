...
export const DialogueBubble: React.FC<DialogueBubbleProps> = ({ message, character, showAvatar = true }) => {
  ...
  return (
    <div className={`flex items-start gap-3 ${isLeft ? '' : 'flex-row-reverse'}`}>
      ...
      <div
        className={`max-w-[70%] rounded-2xl px-4 py-3 shadow-sm mt-2 ${isLeft ? 'rounded-tl-sm' : 'rounded-tr-sm'}`}
        style={{
