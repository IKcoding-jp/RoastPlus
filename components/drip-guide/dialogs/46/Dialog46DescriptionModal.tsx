...
export const Dialog46DescriptionModal: React.FC<Dialog46DescriptionModalProps> = ({ isOpen, onClose }) => {
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div {...overlayMotion} className="fixed inset-0 z-[60] bg-black/40" onClick={onClose} />
          <motion.div
            {...dialogMotion}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-edge">
                <h3 className="text-xl font-bold text-ink">4:6メソッドのポイント</h3>
                <IconButton
                  variant="ghost"
                  size="sm"
                  rounded
                ...
                {RECIPE46_DESCRIPTION_SECTIONS.map((section, idx) => (
                  <div key={idx} className="flex gap-4">
                    ...
                    <div className="flex-1 border-b border-edge/30 pb-4">
                      <h4 className="font-bold text-ink mb-1 text-base">{section.title}</h4>
                      <p className="text-[15px] leading-relaxed text-ink-sub whitespace-pre-wrap">{section.content}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="px-6 py-4 bg-ground border-t border-edge flex justify-end">
                <Button variant="coffee" size="sm" onClick={onClose}>
                  閉じる
                </Button>
              </div>
            </div>
