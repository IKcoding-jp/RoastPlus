      ...
      <div className="overflow-x-auto scrollbar-hide pb-1 -mx-2 px-2" ref={scrollContainerRef}>
        <div className="flex gap-2 sm:gap-2 min-w-max">
          {steps.map((step, index) => {
            const stepEndTime = index < steps.length - 1 ? steps[index + 1].startTimeSec : totalDurationSec;
            const isStepCompleted = isManualMode ? index < currentStepIndex : currentTime > stepEndTime;
              ...
              >
                <div
                  className={clsx(
                    ...
                    isCurrent ? 'text-spot-hover' : isStepCompleted ? 'text-ink-sub' : 'text-ink-muted'
                  )}
                >
                  {step.title}
                </div>
                {!isManualMode && (
                  <div
                    className={clsx(
                      ...
                      isCurrent ? 'text-spot' : isStepCompleted ? 'text-ink-sub' : 'text-ink-muted'
                    )}
                  >
                ...
                {step.targetTotalWater && (
                  <div
                    className={clsx(
                      ...
                      isCurrent ? 'text-spot' : isStepCompleted ? 'text-ink-muted' : 'text-ink-muted'
                    )}
                  >
