...
export function Dialog({
  ...
  const cancelBtnClass =
    'flex-1 bg-ground hover:bg-edge text-ink-sub py-2.5 px-4 rounded-xl font-medium transition-colors border border-edge disabled:opacity-50';
  ...
  return (
    <Modal show={isOpen} onClose={onClose} contentClassName={contentClassName} closeOnBackdropClick={!isLoading}>
      <div className="p-6">
        {isDanger && (
          <div className="flex justify-center mb-4">
            <div className="w-12 h-12 rounded-full flex items-center justify-center bg-danger-subtle">
              <Warning size={28} className="text-danger" weight="fill" />
            </div>
          </div>
        )}

        <h3 className={`text-lg font-bold text-ink ${isDanger ? 'text-center' : ''} mb-2`}>{title}</h3>

        {description && <p className={`text-sm text-ink-sub ${isDanger ? 'text-center' : ''} mb-6`}>{description}</p>}

        <div className="flex gap-3">
          <button type="button" onClick={onClose} disabled={isLoading} className={cancelBtnClass}>
            {cancelText}
          </button>
