...
export const QuickAddModal: React.FC<QuickAddModalProps> = ({ isOpen, onClose, workProgress, onAdd, unit }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4 transition-opacity duration-300">
      ...
      >
        ...
        <div className="px-4 py-3 border-b border-edge flex items-center justify-between bg-ground">
          <h3 className="font-bold text-ink truncate pr-4">進捗を記録: {workProgress.taskName}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-4 overflow-y-auto">
          ...
          <div className="mb-6">
            <label className="block text-xs font-bold text-ink-muted uppercase tracking-wide mb-2">
              {workProgress.targetAmount === undefined ? '追加する個数' : `追加する量 (${unit || '個'})`}
            </label>
          ...
          {/* Memo */}
          <div className="mb-6">
            <label className="block text-xs font-bold text-ink-muted uppercase tracking-wide mb-2">メモ (任意)</label>
            <textarea
              value={memo}
          ...
          <Button
            ...
            className={`!rounded-xl shadow-md active:scale-[0.98] ${
