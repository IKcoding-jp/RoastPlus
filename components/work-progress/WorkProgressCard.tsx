      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            {wp.taskName && <h3 className="text-base font-bold text-ink leading-tight">{wp.taskName}</h3>}
            ...
            <select
              ...
              className={`px-2 py-1 text-xs font-medium rounded border focus:outline-none focus:ring-2 focus:ring-spot min-h-[32px] cursor-pointer ${
                wp.status === 'completed'
            ...
            <div>
              ...
              <div className="text-xl font-bold text-ink leading-none flex items-baseline gap-1">
                ...
                <span className="text-sm font-normal text-ink-muted ml-1">
                  / {formatAmount(wp.targetAmount, unit)}
                  {unit}
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <span className="text-xl font-bold text-ink">
                {wp.completedCount || 0}
                <span className="text-sm font-normal text-ink-muted ml-1">個</span>
              </span>
            </div>
            <Button
              variant="primary"
                ...
                .map((entry) => (
                  ...
                  >
                    <div>
                      <span className="font-medium text-ink-sub">
                        {entry.amount >= 0 ? '+' : '-'}
                        {formatAmount(Math.abs(entry.amount), unit)}
                        {unit}
                      </span>
