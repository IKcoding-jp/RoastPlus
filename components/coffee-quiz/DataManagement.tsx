...
export function DataManagement({ onImportSuccess }: DataManagementProps) {
  ...
  return (
    <div className="bg-surface rounded-xl border border-edge p-4">
      <h3 className="text-sm font-semibold text-ink mb-3">データ管理</h3>
      <p className="text-xs text-ink-muted mb-4">データのバックアップや他の端末への移行ができます</p>
        ...
        {message.type && (
          <motion.div
            ...
            className={`mt-3 flex items-center gap-2 text-sm py-2 px-3 rounded-lg ${
              message.type === 'success' ? 'bg-emerald-500/10 text-emerald-700' : 'bg-rose-500/10 text-rose-700'
            }`}
          >
