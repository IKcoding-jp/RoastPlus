...
- ユーザーがPR作成まで依頼した場合、PR作成で作業完了にせず、CI結果を監視する。
- `gh pr checks --watch` またはGitHubコネクタでチェック状態を確認する。
- CIが失敗した場合は、失敗jobとログを確認し、修正できるものは修正してコミット・pushする。
- 修正後は再度CIを監視し、全チェックが通ってマージ可能な状態になるまで繰り返す。
- マージはユーザーが行う。明示依頼なしにAIエージェントはマージしない。
