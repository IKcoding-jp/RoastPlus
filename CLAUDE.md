...
| `superpowers:requesting-code-review` | 完了時のレビュー依頼（fix-issue Phase 6でAI自動判断） |
...
|-------------------|:---:|:---:|:---:|
| `brainstorming` | ❌ 不要（Phase 3で代替） | ❌ 不要（Phase 0で明示呼出） | ✅ 使用 |
| `writing-plans` | ❌ 不要（Working Documentsで代替） | ❌ 不要（Phase 6で代替） | ✅ 使用 |
| `test-driven-development` | ✅ Phase 4で明示呼出 | - | ✅ 使用 |
| `verification-before-completion` | ❌ 不要（Phase 5+7で代替） | - | ✅ 使用 |
| `requesting-code-review` | ❌ 不要（Phase 6でAI自動判断） | - | ✅ 使用 |
| `finishing-a-development-branch` | ❌ 不要（Phase 10で代替） | - | ✅ 使用 |
...
- fix-issue Phase 3（説明+計画）は**軽量化**（ブレインストーミング済みのため要点のみ）
...
| PR完了時（Phase 9） | **変更影響判定で該当ドキュメントのみレビュー**（スキップにも理由明記） |
...
UIを生成・修正する際は必ず `DESIGN.md` を参照すること。
新しいコンポーネントを作る場合も、`DESIGN.md` に定義されたカラー・スペーシング・コンポーネントスタイルに従うこと。
...
| **/fix-issue** | 実装開始時 | Working読込→実装→検証（lint/build/unit/E2E）→レビュー（AI判断）→PR→即マージ |
