  ...
  for (const team of teams) {
    ...
    for (const taskLabel of taskLabels) {
      ...
      if (unassignedMembers.length > 0) {
        ...
        // 公平性を考慮して、最近割り当てられていない、かつ連続割り当てでないメンバーから選択
        let availableMembers = teamMembers.filter(
          (m) => !recentMemberIds.includes(m.id) && m.id !== currentAssignment
        );

        // 候補がいない場合は、連続割り当てを避けるだけ
        if (availableMembers.length === 0) {
          availableMembers = teamMembers.filter(
            (m) => m.id !== currentAssignment
          );
        }

        // それでも候補がいない場合は、すべてのメンバーから選択
        const finalCandidates =
          availableMembers.length > 0 ? availableMembers : teamMembers;
