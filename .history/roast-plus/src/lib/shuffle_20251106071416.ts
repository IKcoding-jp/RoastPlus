...
export function shuffleAssignments(
  ...
  for (const team of teams) {
    ...
    // ラベルの数がメンバーの数より少ない場合、空のラベルを追加
    const effectiveLabels: Array<{ id: string; leftLabel: string; rightLabel: string | null }> = [...taskLabels];
    const neededLabels = Math.max(taskLabels.length, teamMembers.length);
    
    // 足りない分の空のラベルを追加
    for (let i = taskLabels.length; i < neededLabels; i++) {
      effectiveLabels.push({
    ...
    // ラベルごとに1人のメンバーを割り当て
    for (const taskLabel of effectiveLabels) {
