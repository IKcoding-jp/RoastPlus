...
- 通常の業務スケジュール（朝礼、お昼休み、おそうじ、終礼など）
- 「予熱オン」「◯回目」「アフターパージ」「チャフのお掃除」以外の内容

【ローストスケジュール（roastSchedules）】
以下のキーワードを含む項目はローストスケジュールとして分類してください：
- 「予熱オン」→ isRoasterOn: true
- 「1回目」「2回目」など（数字+回目）→ isRoast: true, roastCount: 数字
- 「アフターパージ」→ isAfterPurge: true
- 「チャフのお掃除」→ isChaffCleaning: true
