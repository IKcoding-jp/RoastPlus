...
JSONのみを返してください。説明文は不要です。`;
  ...
  try {
    ...
    // ローストスケジュールのキーワードを定義（より包括的に）
    const roastKeywords = [
      '予熱オン', '予熱', '回目', 'アフターパージ', 'チャフのお掃除', 'チャフ',
      'ローストスケジュール', 'ロースト1回目', 'ロースト2回目', 'ロースト3回目', 'ロースト4回目', 'ロースト5回目',
      '焙煎1回目', '焙煎2回目', '焙煎3回目', '焙煎4回目', '焙煎5回目',
      '1回目', '2回目', '3回目', '4回目', '5回目',
      '1回', '2回', '3回', '4回', '5回'
    ];

    // TimeLabel形式に変換（idとorderを追加）
    // ローストスケジュールのキーワードを含む項目は除外（より厳密にチェック）
    const filteredTimeLabelsData = timeLabelsData.filter((item: any) => {
      const content = (item.content || '').toLowerCase();
      // キーワードが含まれているかチェック
      const containsRoastKeyword = roastKeywords.some(keyword => 
        content.includes(keyword.toLowerCase())
      );
      // 数字+回目のパターンもチェック（例：「1回目」「2回目」など）
      const containsRoastPattern = /\d+回目?/.test(content);
      
      // ローストスケジュールのキーワードを含む場合は除外
      return !containsRoastKeyword && !containsRoastPattern;
    });
