...
Phase 5.5: コードレビュー(任意)
   └─ レビュー実施確認 → code-reviewスキル → 問題修正
...
PR作成後、マージ前にレビュー実施するか**AskUserQuestionツールで確認**:

```
🔍 コードレビュー
PRを作成しました。マージ前にコードレビューを実行しますか？
（小さな修正ならスキップ可）
```

**承認された場合:**
1. `code-review`スキルを呼び出してレビュー実施
2. 問題が指摘された場合 → 修正してコミット＆PRを更新
3. 問題なし → Phase 6へ

**スキップされた場合:** そのままPhase 6へ進む。
