...
- **キャッシュ戦略**: Network First（常にネットワーク優先、失敗時のみキャッシュから返す）

### プリキャッシュ方針（2026-06-12 決定）

プリキャッシュ（SWインストール時に事前保存する対象）は**最小アプリシェルのみ**とし、未訪問ページのオフライン表示は保証しない。

- **業務前提**: 現場iPadは常時WiFi接続で、オフライン利用はほぼ発生しない。SWキャッシュは一時的な通信断への保険と位置づける。
- **技術理由**: 静的エクスポート＋クライアントレンダリング構成では、HTMLをプリキャッシュしてもページ固有のJSチャンクがなければ未訪問ページはオフラインで機能しない。完全対応にはビルド時のキャッシュマニフェスト生成（next-pwa / Workbox等）が必要で、ADR-007と矛盾するため採用しない。
- **挙動**: 一度訪問したページは実行時キャッシュ（上限80件・古い順に削除）でオフラインでも表示できる。未訪問ページはオフライン時にSPAフォールバック（`/index.html`）へ落ちる。
- プリキャッシュ対象の一覧は `public/sw.js` の `PRECACHE_URLS` が唯一の正。`lib/pwa/sw.test.ts` が本方針との一致と実在ページとの突き合わせを機械チェックしており、一覧を変更する場合は本節とテストを同時に更新する。

### キャッシュバージョン運用ルール

- バージョンは `public/sw.js` の `SW_VERSION`（単一の数値）で管理し、`CACHE_NAME` / `RUNTIME_CACHE` はそこから導出する。番号の不一致は `lib/pwa/sw.test.ts` が検知する。
- **`public/sw.js` を変更したら `SW_VERSION` を必ず +1 する**。上げ忘れると古い画面が配信され続ける事故につながる。
- バージョンを上げると activate 時に旧バージョンのキャッシュは全削除される。実行時キャッシュもリセットされるが、常時WiFi前提のため業務影響は軽微。
...
- ドキュメント所有者のみ編集可能
- 読み取りは認証済みユーザーに限定
- 生産記録（`productionRecords`）はフィールドレベル検証あり（許可フィールドのホワイトリスト・数値範囲・文字列長・日付形式。クライアント側 `lib/productionRecords.ts` の検証に加えたRules独自の安全弁）
- 許可ルールでは `{document=**}` のような全階層マッチを使わず、実際に使う階層構造のみ許可する（deny-all の `{document=**}` は例外）
