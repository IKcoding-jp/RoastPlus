...
> **追記（2026-05-30, 実装中の方針変更）:** 当初「クリスマスモードのロゴはデザイン変更しない」としていたが、通常ロゴ刷新後に実機で確認したところ、クリスマス専用ロゴ（ツリーアイコン＋Playfair斜体＋光彩＋金下線）が刷新後の通常ロゴと不釣り合いになった。ユーザー判断で、クリスマス専用ロゴ分岐を廃止し**通常ロゴと同一のワードマークに一本化**した。クリスマステーマでは CSS テーマトークン（`--header-text`＝クリーム、`--header-accent`＝金）が切り替わるため、同じマークアップで自動的にクリスマス配色になる。ヘッダー下部の金アクセントラインは維持。詳細は実装計画の Task 6 を参照。
