...
beanAmountGram: 20
totalWaterGram: 150
totalDurationSec: 150        // 2:30。最終ステップ(急冷)開始は120秒。最終ステップ開始 < totalDurationSec を満たし、一覧の所要時間も2:30に保つ
...
| 5 | step-5 | 120 | 氷で急冷 | （なし） | 落ち切った濃いコーヒーを氷（1人前あたり60〜80g）入りの容器に注ぎ、一気に冷やしてグラスへ |

- 人前を増やすと湯量（45/90/120/150）が倍率計算される。氷量は説明文テキストのため倍率対象外。
- `totalDurationSec = 150`（2:30）。注ぎ終わり(step-4, 90秒)→落ち切り→120秒(2:00)で急冷開始→150秒(2:30)で完了、という流れ。記事の「2:00で落ち切り」と整合する。
- 自動モードは `useRunnerTimer` が `next >= totalDurationSec` で完了するため、最終ステップ（急冷）が画面表示されるには `最終ステップ開始(120) < totalDurationSec(150)` を満たす必要がある。同時に、一覧の所要時間表示（`totalDurationSec` をそのまま分秒変換）を2:30に保つため、`totalDurationSec` は最終ステップ開始より後かつ広告値どおりの150とする。
